...
export default function RoomManager() {
  ...
  const totalCapacity = totalDesks * deskCapacity;
    ...
    <div className="space-y-6">
      ...
      {isAddingRoom && (
        <div className="flex flex-col md:flex-row gap-6">
          ...
          {/* Layout Preview */}
          <div className="flex-1 bg-gray-50 p-6 rounded-lg shadow-md h-fit">
            <h2 className="text-lg font-semibold mb-2">Live Layout Preview</h2>
            <div
            ...
            >
              {Array.from({ length: totalDesks }).map((_, i) => (
                <div
                  key={i}
                  className="w-10 h-10 bg-blue-200 border border-blue-500 flex items-center justify-center text-xs rounded"
                >
                  {deskCapacity}
                </div>
              ))}
            ...
            <p className="mt-4 text-sm text-gray-600">
              Total Desks: {totalDesks} | Capacity: {totalCapacity}
            </p>
          </div>
        </div>
      )}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Desk Capacity</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Class Capacity</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {rooms.map((room) => (
              <tr key={room.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">{room.deskCapacity}</td>
